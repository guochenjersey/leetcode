...
/**
 * TODO verify in leet Given a string, determine if it is a palindrome, considering only
...
public class Kata125 {
  ...
  public boolean isPalindrome(String s) {
    ...
    while (head <= tail) {
      if (Character.toUpperCase(filteredString.charAt(head))
          == Character.toUpperCase(filteredString.charAt(tail))) {
        ++head;
        --tail;
      } else {
        return false;
