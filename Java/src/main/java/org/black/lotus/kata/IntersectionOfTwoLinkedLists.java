 ...
 *
 * <p>For example, the following two linked lists: Input: intersectVal = 8, listA = [4,1,8,4,5],
 * listB = [5,0,1,8,4,5], skipA = 2, skipB = 3 Output: Reference of the node with value = 8 Input
 ...
 *
 * <p>Input: intersectVal = 2, listA = [0,9,1,2,4], listB = [3,2,4], skipA = 3, skipB = 1 Output:
 * Reference of the node with value = 2 Input Explanation: The intersected node's value is 2 (note
 * that this must not be 0 if the two lists intersect). From the head of A, it reads as [0,9,1,2,4].
...
public class IntersectionOfTwoLinkedLists {
  ...
  public ListNode getIntersectionNode(ListNode headA, ListNode headB) {
    ...
    while (bLength != 0) {
      if (longer.val == shorter.val && longer.val != 0) {
        return longer;
      } else {
        longer = longer.next;
