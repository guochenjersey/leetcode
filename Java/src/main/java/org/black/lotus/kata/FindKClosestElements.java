 ...
 * always preferred.
 *
 * <p>Example 1: Input: [1,2,3,4,5], k=4, x=3 Output: [1,2,3,4] Example 2: Input: [1,2,3,4,5], k=4,
 ...
 *
 * <p>注意 最接近的那个数字必须满足以下条件中的一个 1) 和MIDDLE相等. 2) < MIDDLE && > middle - 1 3) > middle && < middle + 1
 */
@FirstRound
@LeetCode
  ...
  public List<Integer> findClosestElements(int[] nums, int kElements, int target) {
    ...
    while (start + 1 < end) {
      ...
      if (nums[middle] == target) {
        ...
        minDiffIndex =
            ((target - nums[middle]) > (nums[middle + 1] - target)) ? middle + 1 : middle;
        break;
      } else if (nums[middle] > target && nums[middle - 1] < target) {
        minDiffIndex =
            ((nums[middle] - target) > (target - nums[middle - 1])) ? middle - 1 : middle;
        break;
