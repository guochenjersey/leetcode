...
/**
 * 一致性哈希的关键. 1. 当分布式集群中有新的机器加入, 通过和对象存储一样的哈希算法, 把机器也映射到哈希环中. 通常可以对机器IP或者机器别名做法哈希算法的输入值.
 * 2. java int 是有符号数,那么我们可以把整个环的取值空间想象成 -2 31次方,到2的31次方减去1 3. 虚拟节点的目的是让每个机器在整个HASH环上,更均匀的接受存储对象.
 */
@ConsistentHash
public class ConsistentHashDemo<T> {
  ...
  public CacheMachine findMachine(T t) {
    ...
    int targetHashVal = murMurHashFunction.newHasher().putInt(hashCode()).hash().asInt();
    // 试着顺时针查找,如果没有,就找最小的
  ...
  static class CacheMachine {
    ...
    CacheMachine(String cacheIP, String cacheAlias) {
