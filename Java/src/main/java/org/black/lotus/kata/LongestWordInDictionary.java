 ...
 * <p>它有3个基本性质：
 *
 * <p>1.根节点不包含字符，除根节点外每一个节点都只包含一个字符。
 *
 * <p>2.从根节点到某一节点，路径上经过的字符连接起来，为该节点对应的字符串。
 *
 * <p>3.每个节点的所有子节点包含的字符都不相同。
 *
 * <p>
 *
 * <p>
 *
 * <p>Trie中每个节点有一个特殊标记作为结束符号，通过该标记可以判断当前节点是否是一个字符串的终结节点。
 */
@NotAccepted
@Google
@Important
@org.black.lotus.marker.Trie
...
class TrieNode {
  char c;
  int end; // end非零表示这是一个终结节点，且end-1对应的就是该word在words中的索引,非常重要
