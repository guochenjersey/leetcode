 ...
 * nodes. Return null if LCA does not exist. Notice node A or node B may not exist in tree.
 */
@LintCode
@FirstRound
@NotAccepted
...
public class LCAIII {
  ...
  private void dfs(TreeNode lowerBound, TreeNode upperBound, TreeNode node, Result result) {
    if (node == null) {
      return;
    }
